...
public class LogicalPlan {
    ...
    public OpIterator physicalPlan(TransactionId t, Map<String, TableStats> baseTableStats, boolean explain)
        ...
        while (tableIt.hasNext()) {
            ...
            // generate table's histogram and get its statisics
            String baseTableName = Database.getCatalog().getTableName(table.t);
            statsMap.put(baseTableName, baseTableStats.get(baseTableName));
            // set init filterSelectivities, Scan is full pass, so 1.0
            filterSelectivities.put(table.alias, 1.0);

        }

        // iterate all filters, all filters are one table operator, so just wrap the
        // new filter on table's previous PlanNode
        for (LogicalFilterNode lf : filters) {
            // get table's previous PlanNode
            ...
            // generate phsycal filter
            // wrap the previous subplan with new filter
            subplanMap.put(lf.tableAlias, new Filter(p, subplan));

            // update filterSelectivities, roughly is sel0 * sel1 * sel2 *....
        ...
        for (LogicalJoinNode lj : joins) {
            ...
            // table name deference, some tables refer to another tables by equviMap,
            // another tables refers to real node in subplanMap
            ...
            OpIterator j;
            j = JoinOptimizer.instantiateJoin(lj, plan1, plan2); // generate physical join
            subplanMap.put(t1name, j); // table1 refers to node j in subplanMap

            /*
             * pretty intersting, make table2 referes to table1 in equivMap
             * if want to find table2's node, get table1 by equiMap and then get node by
             * subplanMap. By this way, we just need to update one table_i directly
             * refering to the node after join...cause other tables are indirectly refers
             * this node though table_i
             * xxxxxxxxxx table_i xxx ---> xxx node
             * xxxxxxxxxxxx ^ xxxx subplanMap
             * xx ----------|------------
             * xx | xxxxxxx | xxxxxxxxxx | xxx equivMap
             * table1 xxx table4 xxxxx table6
             */
            if (!isSubqueryJoin) {
        ...
        for (int i = 0; i < selectList.size(); i++) {
            ...
            // if select [gbfield, afield], hasAgg and groupByField != null and
            // si[0].aggOp == null and si[1].aggOp != null , enter 2nd if (0->gbfield) and
            // then 1st if (1->afield)
            // if select [afield], hasAgg and groupByField == null and si[0].aggOp != null,
            // only enter 1st if (0->afield)
            if (si.aggOp != null) {
                ...
                // select a perticular field, e.g. select t1.c2
            } else {
                TupleDesc td = node.getTupleDesc();
                int id;
                try {
                    id = td.fieldNameToIndex(si.fname); // find t1.c2 in the top node tupleDesc
                } catch (NoSuchElementException e) {
                    throw new ParsingException("Unknown field " + si.fname + " in SELECT list");
                }
                outFields.add(id);
        ...
        // wrapper aggregate on top of final join node, it is a one table operator
        if (hasAgg) {
            TupleDesc td = node.getTupleDesc();
            Aggregate aggNode;
            try {
        ...
        // wrapper orderby on toppest, it is a one table operator, order the final
        // result
        if (hasOrderBy) {
