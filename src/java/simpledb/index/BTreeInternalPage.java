 ...
 * 
 * format:
 * parent_pointer, childpage_category (int)
 * slot0***slot1***slot2***slot3 …… ***slotN (bit)
 * ********key1****key2****key3 …… ****keyN (INT_LEN/STRING_LEN)
 * child0**child1**child2**child3 …… **childN (int)
 * 
 * key0 is always null in BTreeInternalPage, so don't need to store it
 * 
 * ***************key{i}
 * child{i-1}*****child{i}
 * 
 * child{i-1} is the left child of key{i}, and child{i} is the right child of
 * key{i}
 *
 * @see BTreeFile
 * @see BufferPool
...
public class BTreeInternalPage extends BTreePage {
	...
	private void deleteEntry(BTreeEntry e, boolean deleteRightChild) throws DbException {
		...
		if (deleteRightChild) {
			...
			// found its left child (marked used) and delete it
	...
	public void updateEntry(BTreeEntry e) throws DbException {
		RecordId rid = e.getRecordId(); // find entry by its RecordId, so entry must points to this page
										// usually, get the entry first, then set key or children and updateEntry
		if (rid == null)
		...
		// the key of entry may be updated, check its validation, must >= left and <=
		// right.
	...
	public void insertEntry(BTreeEntry e) throws DbException {
		...
		for (int i = 0; i < numSlots; i++) {
			if (isSlotUsed(i)) {
				// try to insert (KEY, C0, C1)
				// children[i] == e.getLeftChild().getPageNumber() : insert on the right side
				// key{i-1}***key{i}***KEY***key{i+1}
				// *c{i-1}***c{i}/C0***C1*****c{i+1}
				// just need to shift left or right to the empty slot and get a empty slot here,
				// insert (KEY, C1)
				//
				// children[i] == e.getRightChild().getPageNumber() : insert on the left side
				// key{i-1}******KEY********key{i}
				// ***C0*******c{i-1}/C1******c{i}
				// override origin c{i-1} of key{i-1}
				// so first override c{i-1} -> C0, then shift left entries or right entries to
				// the empty slot and get an empty slot here, insert (KEY, C1)
		...
		if (emptySlot < lessOrEqKey) {
			// get an empty slot at lessOrEqKey, origin lessOrEqKey is shifted left, we have
			// (origin lessOrEqKey, goodSlot)
			for (int i = emptySlot; i < lessOrEqKey; i++) {
				moveEntry(i + 1, i);
			}
			goodSlot = lessOrEqKey;
		} else {
			// get an empty slot at lessOrEqKey+1, lessOrEqKey is not shifted, we have
			// (origin lessOrEqKey, goodSlot)
		...
		// insert (KEY, C1) in the empty slot
