...
public class BTreeFile implements DbFile {
	...
	private BTreeLeafPage findLeafPage(TransactionId tid, Map<PageId, Page> dirtypages, BTreePageId pid,
		...
		while (itr.hasNext()) {
			...
			// left must be <=, while right may > or >=
			if (f.compare(Op.LESS_THAN_OR_EQ, entry.getKey()))
				return findLeafPage(tid, dirtypages, entry.getLeftChild(), perm, f);
		}
		// no key is matched, go to the final child
		if (entry != null)
	...
	public BTreeLeafPage splitLeafPage(TransactionId tid, Map<PageId, Page> dirtypages, BTreeLeafPage page, Field field)
		...
		// move half tuples in the leftpage into rightpage
		Iterator<Tuple> itr = page.reverseIterator();
		Tuple tuple = null;
		int numTuple = page.getNumTuples();
		for (int i = 0; i < numTuple / 2; ++i) {
			if (itr.hasNext()) {
				tuple = itr.next();
				page.deleteTuple(tuple); // MUST delete first and then insert....
											// bcz deleteTuple use RecordId to find the tuple in the page, if
											// insertTuple, its RecordId will be set in the new page, make delete fails
				rightPage.insertTuple(tuple);
			}
		}

		// adjuts sibling_pointers
		if (page.getRightSiblingId() != null) {
			BTreeLeafPage originRightPage = (BTreeLeafPage) getPage(tid, dirtypages, page.getRightSiblingId(),
		...
		// get the right-most tuple in the leftpage and put this key into the parent
		tuple = (itr.hasNext()) ? itr.next() : null;
		assert tuple != null : "Splite leaf page and get null meddile tuple";
		Field keyFieldValue = tuple.getField(keyField);
		BTreeEntry entry = new BTreeEntry(keyFieldValue, page.getId(), rightPage.getId());
		BTreeInternalPage parent = getParentWithEmptySlots(tid, dirtypages, page.getParentId(), keyFieldValue);
		parent.insertEntry(entry);
		updateParentPointers(tid, dirtypages, parent); // adjust parent pointers in the new right page...

		// return the page where keyField is in (left-most)
		if (field.compare(Op.LESS_THAN_OR_EQ, keyFieldValue))
			return page;
		else
			return rightPage;
	}
	...
	public BTreeInternalPage splitInternalPage(TransactionId tid, Map<PageId, Page> dirtypages,
		...
		// get the right-most entry in the leftpage and put this key into the parent
		BTreeEntry keyEntry = itr.next();
		page.deleteKeyAndRightChild(keyEntry);

		keyEntry.setLeftChild(page.getId());
		keyEntry.setRightChild(rightPage.getId());

		BTreeInternalPage parent = getParentWithEmptySlots(tid, dirtypages, page.getParentId(), keyEntry.getKey());
		parent.insertEntry(keyEntry);

		// some entry moves into rightpage, their child's parent_pointers need to be
		// adjusted to rightPage
		updateParentPointers(tid, dirtypages, rightPage);
		// adjust parent pointers in the new right page...
		updateParentPointers(tid, dirtypages, parent);

		if (field.compare(Op.LESS_THAN_OR_EQ, keyEntry.getKey()))
			return page;
		else
			return rightPage;
	}
	...
	private void updateParentPointers(TransactionId tid, Map<PageId, Page> dirtypages, BTreeInternalPage page)
		...
		BTreeEntry e = null;
		// make every child of pid points to it
		while (it.hasNext()) {
			e = it.next();
	...
	public void stealFromLeafPage(BTreeLeafPage page, BTreeLeafPage sibling,
		...
		int num2remove = (page.getNumTuples() + sibling.getNumTuples()) / 2 - page.getNumTuples();
		...
		assert tuple != null : "meiddle tuple of leaf is null after steal";
	...
	public void stealFromLeftInternalPage(TransactionId tid, Map<PageId, Page> dirtypages,
		...
		int num2remove = (page.getNumEntries() + leftSibling.getNumEntries()) / 2 - page.getNumEntries();

		Iterator<BTreeEntry> itr = leftSibling.reverseIterator();
		BTreeEntry entry = null;
		if (itr.hasNext())
			entry = itr.next();
		assert entry != null : "Entry is null when steal from left internal page";
		BTreeEntry parent2rightEntry = new BTreeEntry(parentEntry.getKey(), entry.getRightChild(),
				page.iterator().next().getLeftChild());
		page.insertEntry(parent2rightEntry);

		for (int i = 0; i < num2remove - 1; ++i) {
	...
	public void stealFromRightInternalPage(TransactionId tid, Map<PageId, Page> dirtypages,
		...
		// the corresponding parent entry. Be sure to update the parent
		// pointers of all children in the entries that were moved.
		int num2remove = (page.getNumEntries() + rightSibling.getNumEntries()) / 2 - page.getNumEntries();

		Iterator<BTreeEntry> itr = rightSibling.iterator();
		BTreeEntry entry = null;
		if (itr.hasNext())
			entry = itr.next();
		assert entry != null : "Entry is null when steal from left internal page";
		BTreeEntry parent2leftEntry = new BTreeEntry(parentEntry.getKey(),
				page.reverseIterator().next().getRightChild(),
				entry.getLeftChild());
		page.insertEntry(parent2leftEntry);
	...
	public void mergeLeafPages(TransactionId tid, Map<PageId, Page> dirtypages,
		...
		// adjust sibling_pointers
		if (rightPage.getRightSiblingId() != null) {
			BTreeLeafPage newRigthPage = (BTreeLeafPage) getPage(tid, dirtypages, rightPage.getRightSiblingId(),
					Permissions.READ_WRITE);
			leftPage.setRightSiblingId(newRigthPage.getId());
			newRigthPage.setLeftSiblingId(leftPage.getId());
		} else {
			leftPage.setRightSiblingId(null);
		}

		// clear the bitmap of rightpage for reuse
	...
	public void mergeInternalPages(TransactionId tid, Map<PageId, Page> dirtypages,
		...
		BTreeEntry entry = itr.next();
		// put parent key into left child
		BTreeEntry parent2leftEntry = new BTreeEntry(parentEntry.getKey(),
				leftPage.reverseIterator().next().getRightChild(), entry.getLeftChild());
		leftPage.insertEntry(parent2leftEntry);
		// move all entries of right page into left
		...
		// all right page entries are in left page now, their children should be set to
		// the leftpage...do update here
	...
	public int getEmptyPageNo(TransactionId tid, Map<PageId, Page> dirtypages)
		...
		if (headerId == null) {
			synchronized (this) {
				// create the new page (append in the last)
				BufferedOutputStream bw = new BufferedOutputStream(
						new FileOutputStream(f, true));
				byte[] emptyData = BTreeInternalPage.createEmptyPageData();
				bw.write(emptyData);
				bw.close();
				emptyPageNo = numPages(); // the first page we count is 1, exclude ROOTPRT... so numPages stands for its
											// PGNo, don't -1
			}
		}
