 ...
 *
 * format:
 * parent_pointer, leftSibling_pointer, rightSilbling_pointer (int)
 * slot0**slot1**slot2**slot3 …… slotN (1bit)
 * key0***key1***key2***key3**……**keyN (INT_LEN/STRING_LEN)
 * t0*****t1*****t2*****t3****……**tN (tuple_size)
 * 
 * @see BTreeFile
 * @see BufferPool
 *
...
public class BTreeLeafPage extends BTreePage {
	 ...
	 * 
	 * @param id   - the id of this page, NOTE: pid stores not only tableId and
	 *             PgNo, but also its PGCategory, we can find its category through
	 *             its pid.pgcatg()
	...
	public void deleteTuple(Tuple t) throws DbException {
		RecordId rid = t.getRecordId(); // find tuple through its RecordId, so this tuple must points to this page
										// usually, get the tuple first and then delete...
		if (rid == null)
		...
		t.setRecordId(null); // clear its RecordId
	}
