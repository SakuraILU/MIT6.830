...
public class LogFile {
        ...
        synchronized (Database.getBufferPool()) {
            synchronized (this) {
                ...
                while (els.hasNext()) {
                    ...
                    // System.out.println(key + "---" + tidToFirstLogRecord.get(key));
                }
    ...
    public synchronized void logTruncate() throws IOException {
        ...
        logNew.seek(0);
        logNew.writeLong((cpLoc - minLogRecord) + LONG_SIZE); // LONG_SIZE: the pointer to checkpoint at beginning
        ...
        // delete origin logfile and save the new file as logfile, set raf to the end of
        // the new logfile
        raf.close();
        logFile.delete();
    ...
    public void rollback(TransactionId tid)
        ...
        synchronized (Database.getBufferPool()) {
            synchronized (this) {
                ...
                currentOffset = raf.getFilePointer();
                raf.seek(start);
                HashSet<PageId> pageRecoveried = new HashSet<PageId>();
                while (true) {
                    try {
                        int cpType = raf.readInt();
                        long cpTid = raf.readLong();
                        ...
                        // the end of every reocord is a pointer to the begining of this record
                        raf.readLong();
                    } catch (EOFException e) {
                        break;
                    }
                }
                // rollbacked this tid, update tidToFirstLogRecord for this tid
                tidToFirstLogRecord.remove(tid.getId());
            ...
            try {
                ...
                switch (cpType) {
                    ...
                    case UPDATE_RECORD:
                        ...
                        System.out.println((raf.getFilePointer() - (LONG_SIZE + LONG_SIZE)) + ": TID: " + cpTid);
